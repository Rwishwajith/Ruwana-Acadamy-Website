...
function SetGridSortIconAndFirstCol() {
    ...
    $('tr th a', tbl).each(function () {
        ...
        if (hdr.indexOf(' ▲', hdr.length - 2) != -1) {
            a.html(hdr.substring(0, hdr.length - 2) + ' <span class="glyphicon glyphicon-sort-by-attributes"></span>');
        }
        if (hdr.indexOf(' ▼', hdr.length - 2) != -1) {
            a.html(hdr.substring(0, hdr.length - 2) + ' <span class="glyphicon glyphicon-sort-by-attributes-alt"></span>');
        }
    });
...
function AlertIt(msg) {
    ...
    dlg.dialog({
        ...
        open: function (event, ui) {
            ...
            $(".ui-dialog", dlg.parent().parent()).find(".ui-widget-header").css("background", "#2049EE");
